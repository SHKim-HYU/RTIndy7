...
Bullet_Indy7::Bullet_Indy7(class b3RobotSimulatorClientAPI* sim, int robotId){
	this->sim = sim;
...
void Bullet_Indy7::set_torque(JVec  torques ,JVec  max_torques){
...
void Bullet_Indy7::reset_q(JVec q){
	for (int i = 0; i<q.size();i++){
		sim->resetJointState(this->robotId,this->actuated_joint_id.at(i),q[i]);
	}
}
void Bullet_Indy7::apply_ext_FT(JVec FT){
	btVector3 force ;
	btVector3 torque ;
	torque[0] = -FT(0);
	torque[1] = -FT(1);
