...
public:
	...
	inline void updateTransformTP(float t)
	{
		memcpy(_buf + PACKET_DATA_OFFSET, &t, sizeof(float));
		_makeSocketTransformData();
	...
	inline void updateTransformMonZero(float t)
	{
		memcpy(_buf + PACKET_DATA_OFFSET, &t, sizeof(float));
		_makeSocketTransformData();
...
private:
	enum
	{
		...
		TRANSFORM_PACKET_DATA_LENGTH = sizeof(float) * 12 * SubsysType::NUM_BODIES, // 12 for HTransforma data + q() + p()
		MONITOR_PACKET_DATA_LENGTH = 2 + sizeof(float) * (1*SubsysType::NUM_BODIES), // robot.q()
