...
int NRMK_Master::getAxisEcatStatus(unsigned int AxisIdx, unsigned int & State)
{
    ...
    for (int i=0; i<NUM_NRMK_INDY_TOOL_AXES; i++)
        ...
        // if (_RFTECATV1.Index == AxisIdx-1)
        // {
        //     ec_slave_config_state_t s;

        //     ecrt_slave_config_state(_RFTECATV1.Config, &s);
...
int NRMK_Master::_activateMaster()
{
    if (_systemVars->_master != NULL)
    {
        ...
        //set_slave_state(_systemVars->_master, _RFTECATV1.Config, EC_STATE_OPERATIONAL);
...
void NRMK_Master::processTxDomain()
{
    ...
    for (int i=0; i<NUM_FT_SENSORS; ++i)
    {
     _RFTECATV1[i].OutParam.FTRawFx = EC_READ_S32(_systemVars->_txDomain_pd + _RFTECATV1[i].offFTRawFx);
     _RFTECATV1[i].OutParam.FTRawFy = EC_READ_S32(_systemVars->_txDomain_pd + _RFTECATV1[i].offFTRawFy);
     _RFTECATV1[i].OutParam.FTRawFz = EC_READ_S32(_systemVars->_txDomain_pd + _RFTECATV1[i].offFTRawFz);
    _RFTECATV1[i].OutParam.FTRawTx = EC_READ_S32(_systemVars->_txDomain_pd + _RFTECATV1[i].offFTRawTx);
    _RFTECATV1[i].OutParam.FTRawTy = EC_READ_S32(_systemVars->_txDomain_pd + _RFTECATV1[i].offFTRawTy);
    _RFTECATV1[i].OutParam.FTRawTz = EC_READ_S32(_systemVars->_txDomain_pd + _RFTECATV1[i].offFTRawTz);
    _RFTECATV1[i].OutParam.FTOverloadStatus = EC_READ_S32(_systemVars->_txDomain_pd + _RFTECATV1[i].offFTOverloadStatus);
    _RFTECATV1[i].OutParam.FTTemperature = EC_READ_S32(_systemVars->_txDomain_pd + _RFTECATV1[i].offFTTemperature);
    }
...
float convert_to_float(int32_t value) {
    return *reinterpret_cast<float*>(&value);
}
void NRMK_Master::readBufferFT(float* data)
{      
    data[0] = convert_to_float(_RFTECATV1[0].OutParam.FTRawFx);
    data[1] = convert_to_float(_RFTECATV1[0].OutParam.FTRawFy);
    data[2] = convert_to_float(_RFTECATV1[0].OutParam.FTRawFz);
    data[3] = convert_to_float(_RFTECATV1[0].OutParam.FTRawTx);
    data[4] = convert_to_float(_RFTECATV1[0].OutParam.FTRawTy);
    data[5] = convert_to_float(_RFTECATV1[0].OutParam.FTRawTz);
}
...
void NRMK_Master::set_slave_state(ec_master_t *master, ec_slave_config_t *sc, ec_state_t state) {
    ec_slave_config_state_t sc_state;
    ec_master_state_t master_state;
    ecrt_master_state(master, &master_state);

    ecrt_slave_config_state(sc, &sc_state);
    while (sc_state.al_state != state) {
        std::cout << "Current state: " << sc_state.al_state << ", Target state: " << state << std::endl;
        if (state == EC_STATE_OPERATIONAL && sc_state.al_state == EC_STATE_SAFE_OP) {
            ecrt_slave_config_state(sc, &sc_state);
            if (sc_state.al_state == EC_STATE_SAFE_OP) {
    ...
    std::cout << "Reached state: " << state << std::endl;
}
int NRMK_Master::_initSlaves()
{
   ...
   _RFTECATV1[0].Index = 7;_RFTECATV1[0].Alias = 0; _RFTECATV1[0].Position=7;
    ...
    for (int i=0; i<NUM_FT_SENSORS; i++)
    {
        _RFTECATV1[i].Config = ecrt_master_slave_config(_systemVars->_master, _RFTECATV1[i].Alias, _RFTECATV1[i].Position, RobotusFT_VendorID, Robotus_ProductCode);                   
        if (_RFTECATV1[i].Config == NULL)
        {
            printf("There is no configuration on slave _RFTECATV1!\n");
            return -1;
        }

                            
        if (ecrt_slave_config_pdos(_RFTECATV1[i].Config, EC_END, RFTECATV1_syncs))
        {
            printf("Error in configuring PDOs for slave!\n");
            return -1;
        }
...
int NRMK_Master::_initDomains()
{
    ...
    for (int i=0; i<NUM_FT_SENSORS; i++)
                {
                    _systemVars->_registerTxDomainEntry(RobotusFT_VendorID, Robotus_ProductCode, _RFTECATV1[i].Alias, _RFTECATV1[i].Position, 0x6000, 1, &(_RFTECATV1[i].offFTRawFx), &(_RFTECATV1[i].bitoffFTRawFx)); 
                    _systemVars->_registerTxDomainEntry(RobotusFT_VendorID, Robotus_ProductCode, _RFTECATV1[i].Alias, _RFTECATV1[i].Position, 0x6000, 2, &(_RFTECATV1[i].offFTRawFy), &(_RFTECATV1[i].bitoffFTRawFy)); 
                    _systemVars->_registerTxDomainEntry(RobotusFT_VendorID, Robotus_ProductCode, _RFTECATV1[i].Alias, _RFTECATV1[i].Position, 0x6000, 3, &(_RFTECATV1[i].offFTRawFz), &(_RFTECATV1[i].bitoffFTRawFz)); 
                    _systemVars->_registerTxDomainEntry(RobotusFT_VendorID, Robotus_ProductCode, _RFTECATV1[i].Alias, _RFTECATV1[i].Position, 0x6000, 4, &(_RFTECATV1[i].offFTRawTx), &(_RFTECATV1[i].bitoffFTRawTx)); 
                    _systemVars->_registerTxDomainEntry(RobotusFT_VendorID, Robotus_ProductCode, _RFTECATV1[i].Alias, _RFTECATV1[i].Position, 0x6000, 5, &(_RFTECATV1[i].offFTRawTy), &(_RFTECATV1[i].bitoffFTRawTy)); 
                    _systemVars->_registerTxDomainEntry(RobotusFT_VendorID, Robotus_ProductCode, _RFTECATV1[i].Alias, _RFTECATV1[i].Position, 0x6000, 6, &(_RFTECATV1[i].offFTRawTz), &(_RFTECATV1[i].bitoffFTRawTz)); 
                    _systemVars->_registerTxDomainEntry(RobotusFT_VendorID, Robotus_ProductCode, _RFTECATV1[i].Alias, _RFTECATV1[i].Position, 0x6000, 7, &(_RFTECATV1[i].offFTOverloadStatus), &(_RFTECATV1[i].bitoffFTOverloadStatus)); 
                    _systemVars->_registerTxDomainEntry(RobotusFT_VendorID, Robotus_ProductCode, _RFTECATV1[i].Alias, _RFTECATV1[i].Position, 0x6000, 8, &(_RFTECATV1[i].offFTTemperature), &(_RFTECATV1[i].bitoffFTTemperature));                     
                    
                    
                }

    // _systemVars->_registerTxDomainEntry(RobotusFT_VendorID, Robotus_ProductCode, _RFTECATV1.Alias, _RFTECATV1.Position, 0x6000, 0x01, &(_RFTECATV1.offFTRawFx), &(_RFTECATV1.bitoffFTRawFx)); 
    // _systemVars->_registerTxDomainEntry(RobotusFT_VendorID, Robotus_ProductCode, _RFTECATV1.Alias, _RFTECATV1.Position, 0x6000, 0x02, &(_RFTECATV1.offFTRawFy), &(_RFTECATV1.bitoffFTRawFy)); 
    // _systemVars->_registerTxDomainEntry(RobotusFT_VendorID, Robotus_ProductCode, _RFTECATV1.Alias, _RFTECATV1.Position, 0x6000, 0x03, &(_RFTECATV1.offFTRawFz), &(_RFTECATV1.bitoffFTRawFz)); 
    // _systemVars->_registerTxDomainEntry(RobotusFT_VendorID, Robotus_ProductCode, _RFTECATV1.Alias, _RFTECATV1.Position, 0x6000, 0x04, &(_RFTECATV1.offFTRawTx), &(_RFTECATV1.bitoffFTRawTx)); 
    // _systemVars->_registerTxDomainEntry(RobotusFT_VendorID, Robotus_ProductCode, _RFTECATV1.Alias, _RFTECATV1.Position, 0x6000, 0x05, &(_RFTECATV1.offFTRawTy), &(_RFTECATV1.bitoffFTRawTy)); 
    // _systemVars->_registerTxDomainEntry(RobotusFT_VendorID, Robotus_ProductCode, _RFTECATV1.Alias, _RFTECATV1.Position, 0x6000, 0x06, &(_RFTECATV1.offFTRawTz), &(_RFTECATV1.bitoffFTRawTz)); 
    // _systemVars->_registerTxDomainEntry(RobotusFT_VendorID, Robotus_ProductCode, _RFTECATV1.Alias, _RFTECATV1.Position, 0x6000, 0x07, &(_RFTECATV1.offFTOverloadStatus), &(_RFTECATV1.bitoffFTOverloadStatus)); 
    // _systemVars->_registerTxDomainEntry(RobotusFT_VendorID, Robotus_ProductCode, _RFTECATV1.Alias, _RFTECATV1.Position, 0x6000, 0x08, &(_RFTECATV1.offFTTemperature), &(_RFTECATV1.bitoffFTTemperature)); 
...
//FTsensor
    for (int i=0; i<NUM_FT_SENSORS; i++)
      ecrt_slave_config_dc(_RFTECATV1[i].Config, 0x0000, _systemVars->_cycle_ns, 0, 0, 0);
#ifdef __CB__   
