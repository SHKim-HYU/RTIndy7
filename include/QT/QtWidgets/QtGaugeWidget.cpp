...
void QGaugeWidget::setMinimumSize(int minw, int minh)
{
    QWidget::setMinimumSize(minw, minh);
}

QcBackgroundItem *QGaugeWidget::addBackground(float position)
{
...
QcDegreesItem *QGaugeWidget::addDegrees(float position)
{
...
QcValuesItem *QGaugeWidget::addValues(float position)
{
...
QcArcItem *QGaugeWidget::addArc(float position)
{
...
QcColorBand *QGaugeWidget::addColorBand(float position)
{
...
QcColorBand* QGaugeWidget::addColorBand(float position, QColor fc, float fp, QColor sc, float sp, QColor tc, float tp)
{
    QcColorBand* item = new QcColorBand(this);
    ...
    item->mBandColors.clear();

    pair.first = fc;
    pair.second = fp;
    item->mBandColors.append(pair);
...
QcNeedleItem *QGaugeWidget::addNeedle(float position)
{
...
QcLabelItem *QGaugeWidget::addLabel(float position)
{
...
QcGlassItem *QGaugeWidget::addGlass(float position)
{
...
QcAttitudeMeter *QGaugeWidget::addAttitudeMeter(float position)
{
...
void QGaugeWidget::addItem(QcItem *item,float position)
{
...
int QGaugeWidget::removeItem(QcItem *item)
{
   return mItems.removeAll(item);
}

QList<QcItem *> QGaugeWidget::items()
{
    return mItems;
}


void QGaugeWidget::paintEvent(QPaintEvent */*paintEvt*/)
{
    ...
    QcScaleItem(parent)
{
    ...
    mUnit = QString("%1%2").arg(QChar(0260)).arg("C");
}
...
void QcNeedleItem::setCurrentValue(float value)
{
    ...
    if (mLabel != 0) {
        ...
        QString msg;
        msg = QString::number(mCurrentValue) + " " + mUnit;
        mLabel->setText(msg, false);
    }
    /*
    if (mLabel != 0) {
        //mLabel->setText(QString::number(mCurrentValue), false);
        QString msg;
        if (type == DataUnit::TEMP_C) {
            msg = QString::number(mCurrentValue) + " " + QChar(0260) + "C";
        }
        else if (type == DataUnit::TEMP_F) {
            msg = QString::number(mCurrentValue) + " " + QChar(0260) + "F";
        }
        else if (type == DataUnit::PRESSURE_BAR) {
            msg = QString::number(mCurrentValue) + " " + "bar";
        }
        else if (type == DataUnit::PRESSURE_PSI) {
            msg = QString::number(mCurrentValue) + " " + "psi";
        }
        else if (type == DataUnit::FLOW_LPM) {
            msg = QString::number(mCurrentValue) + " " + "LPM";
        }
        else {
            msg = QString::number(mCurrentValue) + " " + QChar(0260) + "C";
        }
...
void QcNeedleItem::setUnitFormat(QString unit) {
    mUnit = unit;
    update();
}

QString QcNeedleItem::currentUnitFormat() {
    return mUnit;
}
