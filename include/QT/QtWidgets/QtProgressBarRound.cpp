/*
 * QProgressBarRound - a circular progress bar Qt widget.
 *
...
QProgressBarRound::QProgressBarRound(QWidget *parent) :
    QWidget(parent),
    m_min(0), m_max(100),
    m_value(25),
...
void QProgressBarRound::setRange(double min, double max)
{
    m_min = min;
    m_max = max;
...
void QProgressBarRound::setMinimum(double min)
{
    setRange(min, m_max);
}

void QProgressBarRound::setMaximum(double max)
{
    setRange(m_min, max);
}

void QProgressBarRound::setValue(double val)
{
    if (m_value != val)
    {
        if (val < m_min)
...
void QProgressBarRound::setValue(int val)
{
    setValue(double(val));
}

void QProgressBarRound::setNullPosition(double position)
{
...
void QProgressBarRound::setBarStyle(QProgressBarRound::BarStyle style)
{
    if (style != m_barStyle)
    {
        m_barStyle = style;
...
void QProgressBarRound::setOutlinePenWidth(double penWidth)
{
...
void QProgressBarRound::setDataPenWidth(double penWidth)
{
...
void QProgressBarRound::setDataColors(const QGradientStops &stopPoints)
{
...
void QProgressBarRound::setFormat(const QString &format)
{
    if (format != m_format)
    {
        m_format = format;
...
void QProgressBarRound::setFormatWithSize(const QString& format, double fontSizeRatio)
{
    m_fontSizeRatio = fontSizeRatio;
    setFormat(format);    
}

void QProgressBarRound::resetFormat()
{
    m_format = "";
...
void QProgressBarRound::setDecimals(int count)
{
...
void QProgressBarRound::paintEvent(QPaintEvent* /*event*/)
{
...
void QProgressBarRound::drawBackground(QPainter &p, const QRectF &baseRect)
{
    p.fillRect(baseRect, palette().window());
}

void QProgressBarRound::drawBase(QPainter &p, const QRectF &baseRect)
{
    switch (m_barStyle)
    {
    case StyleDonut:
...
void QProgressBarRound::drawValue(QPainter &p, const QRectF &baseRect, double value, double delta)
{
    // nothing to draw
    if (value == m_min)
        return;
...
void QProgressBarRound::calculateInnerRect(const QRectF &/*baseRect*/, double outerRadius, QRectF &innerRect, double &innerRadius)
{
...
void QProgressBarRound::drawInnerBackground(QPainter &p, const QRectF &innerRect)
{
...
void QProgressBarRound::drawText(QPainter &p, const QRectF &innerRect, double innerRadius, double value)
{
    if (m_format.isEmpty())
        return;
    ...
    f.setPixelSize(fontSize* m_fontSizeRatio);
...
QString QProgressBarRound::valueToText(double value) const
{
...
void QProgressBarRound::valueFormatChanged()
{
    m_updateFlags = 0;
...
void QProgressBarRound::rebuildDataBrushIfNeeded()
{
    if (!m_rebuildBrush)
        return;
