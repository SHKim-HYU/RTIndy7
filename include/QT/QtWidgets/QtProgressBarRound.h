...
/**
 * @brief The QProgressBarRound class represents a circular progress bar and maintains its API
 * similar to the *QProgressBar*.
 *
 * ### Styles
 * QProgressBarRound currently supports Donut, Pie, Expand and Line styles. See setBarStyle() for more details.
 *
 * ### Colors
 * Generally QProgressBarRound uses its palette and font attributes to define how it will look.
 *
 ...
 *
 * ### Value text
 * Value text is generally drawn inside the QProgressBarRound using its `font()` and \a QPalette::Text role from its `palette()`.
 *
 ...
 */
class QProgressBarRound : public QWidget
{
    Q_OBJECT
public:    
    explicit QProgressBarRound(QWidget *parent = 0);
    ...
    void setFormatWithSize(const QString& format, double fontSizeRatio = 1.0);
    /**
