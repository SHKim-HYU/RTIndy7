        ...
        : QAbstractMeter(parent)
{
	...
	m_valueFont.setPointSize(16);

	// QWidget
    setSizePolicy (QSizePolicy::Expanding, QSizePolicy::Expanding);
    setWindowTitle(tr("Analog QThermoMeter"));
	resize(200, 200);
	assert(m_max-m_min != 0);

}

void QThermoMeter::initCoordinateSystem(QPainter & painter)
{
...
void QThermoMeter::paintBackground(QPainter & painter)
{
...
void QThermoMeter::paintEvent(QPaintEvent * )
{
	...
	if (valueOffset())
	{
		...
		QString Str = prefix() + QString("%1").arg(value()) + " " + QChar(0260) + "C"; //suffix();
	...
	if (231 < temp  ) height = 231 + OFFSET; // by rtêæ dosz³a do ko?a terometra ale go nie rozwali³a
	if (OFFSET-5 >= height )  height = OFFSET-5; // by nie zeszlo poni¿ej pieciu "pixeli" poni¿ej zera
	// Narysowanie s³upka  wraz z zbiorniczkiem rt?i
    painter.setPen(Qt::NoPen);
	...
	// nie wiem czy to to powoduje krash pod win98 i brak ba?i pod XP.
	// Na³o¿enie szklanej ba?i
	drawBackground();
