/*
 * This is nearly complete Material design QButtonSlide widget implementation in qtwidgets module.
 * More info: https://material.io/design/components/selection-controls.html#QButtonSlidees
...
void QButtonSlide::init() {
    setFont(style.font);
    setObjectName("QButtonSlide");
    /* setup animations */
...
QRect QButtonSlide::indicatorRect() {
...
QRect QButtonSlide::textRect() {
    const auto w = style.indicatorMargin.left() + style.height + style.indicatorMargin.right();
    return ltr(this) ? rect().marginsRemoved(QMargins(w, 0, 0, 0)) : rect().marginsRemoved(QMargins(0, 0, w, 0));
}

QButtonSlide::QButtonSlide(QWidget* parent) : SelectionControl(parent) {
    init();
}

QButtonSlide::QButtonSlide(const QString& text, QWidget* parent) : QButtonSlide(parent) {
    setText(text);
}

QButtonSlide::QButtonSlide(const QString& text, const QBrush& brush, QWidget* parent) : QButtonSlide(text, parent) {
    style.thumbOnBrush = brush.color();
    style.trackOnBrush = brush.color();
}

QButtonSlide::QButtonSlide(const QString& text, const QFont& font, bool chk, bool enable, QWidget* parent) : QButtonSlide(parent) {
    setText(text);
    setFont(font);
    setChecked(chk);
    setEnabled(enable);
...
QButtonSlide::~QButtonSlide() {

}

QSize QButtonSlide::sizeHint() const {
    auto h = style.height;
...
void QButtonSlide::paintEvent(QPaintEvent*) {
