...
void saturationEint(JVec &eint, JVec saturation_value){
    for(int i =0;i<JOINTNUM;i++){
        if(abs(eint(i))> saturation_value(i)){
            if(eint(i)<0) eint(i) = -saturation_value(i);
            else eint(i) = saturation_value(i);
        }
    }
}
...
// RTRArm_task
void setTraj(JVec& q0, JVec& qT, double Tf, double double_gt, int method){
	if(traj_flag ==0){
		q0 = info.act.q;
		traj_flag =1;
	}
...
void setUDPInfo(JVec calcTorq){
	for(int i = 0;i<JOINTNUM;i++){
		udp_info.act.q[i] = info.act.q(i);
		udp_info.act.dq[i] = info.act.dq(i);
		udp_info.act.ddq[i] = info.act.ddq(i);
		udp_info.des.q[i] = info.des.q(i);
		udp_info.des.dq[i] = info.des.dq(i);
		udp_info.des.ddq[i] = info.des.ddq(i);
		udp_info.act.torque[i]=calcTorq(i);
	}
}
void RTRArm_run(void *arg)
{
	...
	while (run)
	{
		...
		if(system_ready)
		{
			readEcatData();
			EncToRad(wc,dt);
			setTraj(q0, qT,  Tf,  double_gt,  method);
			//compute();
			//Robot_Limit();
...
void UDP_run(void *arg){
	rt_task_set_periodic(NULL, TM_NOW, 1e8);	// period = 10 (msec)
    Poco::Net::DatagramSocket server(Poco::Net::SocketAddress("192.168.0.9", SERVER_PORT));
	rt_printf("UDP SERVER START!\n");
    while (true) {
        UDPIndy7Info recv_data;
        Poco::Net::SocketAddress sender;
        server.receiveFrom(&recv_data, sizeof(UDPIndy7Info), sender);
		rt_printf("========================data Recieved!========================\n");
        server.sendTo(&udp_info, sizeof(UDPIndy7Info), sender);
