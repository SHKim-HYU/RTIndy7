...
def w_p_to_Slist(w,p,ROBOT_DOF):
    ...
    for i in range(0,ROBOT_DOF):
      ...
      p_ = p[i];      
      v_ = -np.cross(w_,p_)
      Slist.append([v_[0],v_[1],v_[2],w_[0],w_[1],w_[2]])
...
def AnalyticJacobianBody(M,Blist, thetalist):
    ...
    return Ja
def Adjoint(T):
	ret=np.array([[T[0,0],T[0,1],T[0,2],T[1,3]*T[2,0] - T[2,3]*T[1,0], T[1,3]*T[2,1] - T[2,3]*T[1,1], T[1,3]*T[2,2] - T[2,3]*T[1,2]],
...
def loadURDF(urdf_name):
	...
	for joint in robot.actuated_joints:
		...
		G = np.eye(6)
		G[0:3,0:3] = child_link.inertial.mass*np.eye(3)
		G[3:6,3:6] = child_link.inertial.inertia
		Glist.append(G)
	...
	Blist = Adjoint(mr.TransInv(M))@ Slist
	Mlist = []
...
def main():
    ...
    robot_data = loadURDF(urdf_path)
    output_file = "LR_info.json"
