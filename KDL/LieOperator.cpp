...
bool LieOperator::NearZero(float near){
		if (near < 0.000001)
			return true;
		else
			return false;
...
Matrix3f LieOperator::MatrixLog3(Matrix3f & R)
{
...
Vector3f LieOperator::so3ToVec(Matrix3f& so3mat) {
    Eigen::Vector3f v_ret;
    v_ret << so3mat(2, 1), so3mat(0, 2), so3mat(1, 0);
    return v_ret;
}
/*
Matrix<float, 7, 1> LieOperator::AxisAng6(se3 expc6) {
	Matrix<float, 7, 1> S_theta;
...
Matrix4f LieOperator::MatrixExp6(se3 _s, float _q) {
	Matrix3f so3, SO3;
	Matrix4f SE3;
	Vector3f v;
	float theta;
	...
	if (NearZero(_s.head(3).norm()) && _s.tail(3).norm() == 1) {
		SE3 << Matrix3f::Identity(), v*theta, 0, 0, 0, 1;
		return SE3;
	}
	else {
		SE3 << SO3, (Matrix3f::Identity()*theta + (1 - cos(theta))*so3 + (theta - sin(theta))*so3*so3)*v,
			0, 0, 0, 1;
		return SE3;
	}
}
