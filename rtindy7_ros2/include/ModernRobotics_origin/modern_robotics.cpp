...
namespace mr2 {
	...
	Eigen::MatrixXd TransInv(const Eigen::MatrixXd& transform) {
		auto rp = mr2::TransToRp(transform);
	...
	*/
	Eigen::VectorXd InverseDynamics(const Eigen::VectorXd& thetalist, const Eigen::VectorXd& dthetalist, const Eigen::VectorXd& ddthetalist,
		...
		Vdi.block(3, 0, 3, 1) = - g;
		AdTi[n] = mr2::Adjoint(mr2::TransInv(Mlist[n]));
		...
		for (int i = 0; i < n; i++) {
			Mi = Mi * Mlist[i];
			Ai.col(i) = mr2::Adjoint(mr2::TransInv(Mi))*Slist.col(i);

			AdTi[i] = mr2::Adjoint(mr2::MatrixExp6(mr2::VecTose3(Ai.col(i)*-thetalist(i)))
			          * mr2::TransInv(Mlist[i]));
	...
	Eigen::VectorXd GravityForces(const Eigen::VectorXd& thetalist, const Eigen::VectorXd& g,
		...
		Eigen::VectorXd grav = mr2::InverseDynamics(thetalist, dummylist, dummylist, g,
	...
	Eigen::MatrixXd MassMatrix(const Eigen::VectorXd& thetalist,
		...
		for (int i = 0; i < n; i++) {
			...
			ddthetalist(i) = 1;
			M.col(i) = mr2::InverseDynamics(thetalist, dummylist, ddthetalist,
	...
	Eigen::VectorXd VelQuadraticForces(const Eigen::VectorXd& thetalist, const Eigen::VectorXd& dthetalist,
		...
		Eigen::VectorXd dummyg = Eigen::VectorXd::Zero(3);
		Eigen::VectorXd dummyforce = Eigen::VectorXd::Zero(6);
		Eigen::VectorXd c = mr2::InverseDynamics(thetalist, dthetalist, dummylist,
                             dummyg, dummyforce, Mlist, Glist, Slist);
		return c;
	}
	...
	Eigen::VectorXd EndEffectorForces(const Eigen::VectorXd& thetalist, const Eigen::VectorXd& Ftip,
		...
		Eigen::VectorXd JTFtip = mr2::InverseDynamics(thetalist, dummylist, dummylist,
	...
	Eigen::VectorXd ForwardDynamics(const Eigen::VectorXd& thetalist, const Eigen::VectorXd& dthetalist, const Eigen::VectorXd& taulist,
		...
		Eigen::VectorXd totalForce = taulist - mr2::VelQuadraticForces(thetalist, dthetalist, Mlist, Glist, Slist)
                 							 - mr2::GravityForces(thetalist, g, Mlist, Glist, Slist)
                                             - mr2::EndEffectorForces(thetalist, Ftip, Mlist, Glist, Slist);

		Eigen::MatrixXd M = mr2::MassMatrix(thetalist, Mlist, Glist, Slist);
